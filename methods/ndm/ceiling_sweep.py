...
def main():
    ...
    with tee_to_log(os.path.join(log_dir, f"ceiling_sweep_{run_label}_layers{layers_tag}"
        ...
        # One BuildBatchCache PER SPEC, not one shared across all of --positions_list. This is now
        # belt-and-braces: entities.py's BuildBatchCache used to cache pos_unpadded under
        # (queried, template_id) with no positions_name, so sharing one instance across specs
        # silently reused the FIRST spec's positions for every later one -- it crashed ring:2 with an
        # out-of-bounds grid index (24-token flag_ring1 positions reused for a 144-token full_image
        # build) and, worse, did NOT crash for a full_image/last_token spec, which just returned the
        # wrong columns. That second shape later cost a whole head_trace result set, so the cache now
        # keys positions properly (see BuildBatchCache's docstring) and this split is no longer
        # load-bearing. Kept anyway: it costs one dict per spec. image_extra/gold ARE
        # positions-independent (image decode, gold token ids), so those stay shared.
